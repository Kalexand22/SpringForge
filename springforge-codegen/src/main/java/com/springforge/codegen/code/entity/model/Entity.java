...
public class Entity implements BaseType<Entity> {
  ...
  public PropertyType getIdType() {
    // Si un champ id est explicitement défini, utiliser son type
    if (idField != null) {
      return idField.getType();
    }
    // Sinon chercher un champ marqué comme ID
    return getFields().stream()
            .filter(Property::isPrimary)
            .findFirst()
            .map(Property::getType)
            .orElse(PropertyType.LONG); // Type par défaut
  }

  public boolean hasAudit() {
    return Boolean.TRUE.equals(this.auditable) ||
            (this.listeners != null && this.listeners.stream()
                    .anyMatch(l -> l.getClazz().contains("AuditingEntityListener")));
  }
}